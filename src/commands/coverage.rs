...
impl CoverageResult {
    ...
    pub fn coverage_percent(&self) -> f64 {
        ...
        let covered: usize = self.ranges.iter()
...
pub fn run(cmd: CoverageCommand, cli: &crate::CliConfig) -> anyhow::Result<()> {
    ...
    if cmd.json {
        ...
        for range in &result.ranges {
            let bar_width = (range.count as f64 / result.total_documents.max(1) as f64 * 20.0) as usize;
            let bar: String = "█".repeat(bar_width) + &"░".repeat(20 - bar_width);
            
            println!("{} {:10} [{:3}] {}", range.emoji, range.name, range.count, bar);
        }
