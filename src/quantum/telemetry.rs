...
impl Telemetry {
    ...
    pub fn track_command(&self, name: &str, duration: Duration) {
        ...
        *self.counters.write().entry(format!("cmd:{}", name)).or_insert(0) += 1;
        *self.durations.write().entry(name.to_string()).or_insert(Duration::ZERO) += duration;
    }
    ...
    pub fn track_error(&self, error: &str) {
        ...
        self.events.write().push(event);
        *self.counters.write().entry("errors:total".to_string()).or_insert(0) += 1;
    }
    ...
    pub fn total_duration(&self, command: &str) -> Duration {
        *self.durations.read().get(command).unwrap_or(&Duration::ZERO)
    }
